    <main>
        <div class="order-details-container">
            <h2>Xác nhận đơn hàng thành công</h2>
            <p>Ngày đặt hàng: 30/10/2024 22:32</p>
        </div>
        <div class="order-details">
            <div class="customer-info">
                <p class="order"><strong class="customer-info-col">Mã đơn hàng:</strong> 1</p>
                <p><strong class="customer-info-color">Tên tài khoản:</strong> Hoàng Nho Thơm</p>
                <p><strong class="customer-info-color">Điện thoại:</strong> 0838934160</p>
                <p><strong class="customer-info-color">Email:</strong> thomhnps36607@fpt.edu.vn</p>
                <p><strong class="customer-info-color">Địa chỉ:</strong> Tô Ký, Trung Mỹ Tây, Q12</p>
            </div>
            <div class="order-summary">
                <h3 class="customer-info-col">Chi tiết đơn hàng</h3>
                <div class="order-item">
                    <div class="order-item-name">
                        <div class="image-quantity">
                            <p>1</p><img src="../public/img/pie&tart-01.jpg" alt="Set 6 Tart Mini">
                        </div>
                        <p>Set 6 Tart Mini</p>
                    </div>
                    <strong> 200,000 VNĐ</strong>
                </div>
                <div class="order-item-price">
                    <div class="order-item-price-col">
                        <strong>Tạm tính:</strong>
                        <p>200,000 VNĐ</p>
                    </div>
                    <div class="order-item-price-col">
                        <strong>Mã giảm giá:</strong>
                        <p> 0</p>
                    </div>
                    <div class="order-item-price-col">
                        <strong>Phí vận chuyển:</strong>
                        <p>0</p>
                    </div>
                </div>
                <div class="order-item-price-cols">
                    <strong>Tổng cộng:</strong>
                    <p>200,000 VNĐ</p>
                </div>
                <p>
                    <strong class="customer-info-col">Phương thức thanh toán </strong> <br></br> 
                    Thanh toán khi nhận hàng
                </p>
            </div>
        </div>
    </main>